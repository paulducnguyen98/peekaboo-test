...
def apply_weight_matrix(image: torch.Tensor, weights: torch.Tensor, iterations: int = 1):
    ...
    # Ép kiểu weights về cùng dtype với image
    weights = weights.to(image.dtype)
    ...
    K, K, _, _ = weights.shape
    R = K // 2
    assert weights.shape[2] == H and weights.shape[3] == W, "the image HW dimensions must be the same as the weights"
